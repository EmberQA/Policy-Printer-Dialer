/**
 * The hold audio generators, shared by both carriers (ENG-159 — Subplan 05).
 *
 * Moved here from `twilio/holdAudio.ts` unchanged. Nothing in this file is
 * Twilio-specific: both classes implement `createProcessedStream(stream) => MediaStream`,
 * which is a generic Web Audio contract that happens to match Twilio's `AudioProcessor`
 * plugin interface structurally. Only the ~70-line HOST — the thing that installs a
 * processor onto a call — is carrier-specific, and there is now one host per carrier
 * (`twilio/holdAudio.ts`, `voice/telnyxHold.ts`).
 *
 * `twilio/holdAudio.ts` re-exports these, so its existing tests and imports are
 * unaffected.
 */

/**
 * Structurally identical to `@twilio/voice-sdk`'s `AudioProcessor`, declared locally so
 * the generators don't drag a carrier SDK into the shared layer.
 */
export interface ProcessedStreamSource {
 ...
 */
export class HoldMusicProcessor implements ProcessedStreamSource {
...
export class SilentAudioProcessor implements ProcessedStreamSource {
	private context: AudioContext | null = null;
	private destination: MediaStreamAudioDestinationNode | null = null;
