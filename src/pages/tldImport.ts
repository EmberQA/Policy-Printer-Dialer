...
export function parseTldCsv(csv: string): TldCsvRow[] {
	...
	if (missing.length > 0) {
		throw new Error(
			`This is not a supported TLD export. Missing: ${missing.join(', ')}.`
		);
	}
...
export function findMatchingAgentKey(
	...
	return (
		agents.find((agent) => normalizeText(agent.name) === normalized)?.key ?? ''
	);
}

export function rowsForAgent(
	rows: TldCsvRow[],
...
export function statusSummary(rows: TldCsvRow[]): Array<{
	...
	return [...counts.entries()]
		.map(([status, count]) => ({
			status,
			count,
			mapping: mapTldDisposition(status)
		}))
...
export function loadTldContacts(userId: string): ImportedTldContact[] {
	...
	try {
		...
		if (parsed.version !== STORAGE_VERSION || !Array.isArray(parsed.contacts))
			return [];
...
export function importedContactToActivity(
	...
	return {
		...
		campaign_id: null,
		campaign_name: contact.listName
			? `TLD • ${contact.listName}`
			: 'TLD import',
...
function rowToContact(
	...
	const sourceLeadId =
		row.lead_id.trim() || `${row.vendor_lead_code.trim()}-${index}`;
...
function cleanPhone(value: string): string {
	...
	return numeric.length === 11 && numeric.startsWith('1')
		? numeric.slice(1)
		: numeric;
}

function compareNewestFirst(
	a: ImportedTldContact,
	b: ImportedTldContact
): number {
