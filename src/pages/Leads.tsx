...
export default function Leads() {
  ...
  const onClickToDial = (rowId: string, phone: string) => {
    ...
    setDialingId(rowId);
    setDialError(null);
    // Start through the shared device before leaving this click handler so ringback
    // satisfies autoplay rules, then show the persistent call banner on Dial.
    const attempt = device.startOutbound(dest);
    //     This unmounts Leads but NOT the provider/Device, so the in-flight call +
    //     pending state stay on the persistent Device instance.
    navigate("/dial");
    attempt
...
function ActivityRow({
  ...
  const dialTo = item.caller_phone
    ? normalizeDialInput(item.caller_phone)
    : null;
    ...
    <div className="space-y-5 px-4 py-4">
      ...
      {logging && !effectiveCampaign && (
        <div className="max-w-sm space-y-2">
          <Label>Campaign</Label>
          <Select
            value={pickedCampaign || undefined}
            onValueChange={setPickedCampaign}
          >
            <SelectTrigger className="w-full">
  ...
  return (
    <div className="space-y-5 px-4 py-4">
      ...
      {schema.length > 0 ? (
        ...
        <p className="text-sm text-muted-foreground">
          No form captured for this lead.
        </p>
      )}
      ...
      <div className="grid gap-5 border-t pt-4 md:grid-cols-2">
        <div className="grid gap-2 text-sm">
          ...
          <Row
            label="Disposition"
            value={detail.lead.disposition_label || "—"}
          />
